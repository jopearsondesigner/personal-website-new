<script lang="ts">
  ...
  export let isOpen = false; // Ensure this is used with bind:isOpen in parent component
...
>
  <svg
    class="w-6 h-6 transition-transform duration-300 {isOpen
      ? 'rotate-90'
      : ''}"
    fill="none"
    stroke="currentColor"
  ...
  >
    ...
    >
      ...
      <div class="absolute bottom-8 left-6 right-6">
        ...
        />
        <div class="mt-4 text-sm text-arcadeWhite-300/60 text-center">
          Press ESC to close
        </div>
      </div>
    </div>
