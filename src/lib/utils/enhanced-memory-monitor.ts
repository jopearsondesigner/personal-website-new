...
export class EnhancedMemoryMonitor {
	 ...
	 */
	onMemoryPressure(callback: (snapshot: MemorySnapshot) => void): EnhancedMemoryMonitor {
	 ...
	 */
	onGCDetected(callback: (snapshot: MemorySnapshot) => void): EnhancedMemoryMonitor {
	 ...
	 */
	onLeakSuspected(callback: (snapshot: MemorySnapshot) => void): EnhancedMemoryMonitor {
...
if (browser) {
	...
	setTimeout(() => {
		enhancedMemoryMonitor.start();

		// Set up warning handler
		enhancedMemoryMonitor.onMemoryPressure((snapshot) => {
		...
		enhancedMemoryMonitor.onLeakSuspected((snapshot) => {
			console.warn('Possible memory leak detected');
		});
	}, 1000);

	// Expose to window for debugging
	(window as any).enhancedMemoryMonitor = enhancedMemoryMonitor;
}
