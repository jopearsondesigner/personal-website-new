 ...
 * Improved version that properly accounts for arcade cabinet UI elements
 */
export function calculateOptimalCanvasDimensions(
	...
	// Account for panels in the available width
	let adjustedWidth = availableWidth;
	if (hasLeftPanel) adjustedWidth -= panelWidth;
	if (hasRightPanel) adjustedWidth -= panelWidth;
	...
	// Use higher scaling factors than original to prevent tiny canvas
	if (performanceTier === 'low') {
		// Use 75% of the base resolution for low-end devices (increased from 70%)
		optimalWidth = Math.floor(baseWidth * 0.75);
		optimalHeight = Math.floor(baseHeight * 0.75);
	} else if (performanceTier === 'medium') {
		// Use 90% of the base resolution for medium-tier devices (increased from 85%)
		optimalWidth = Math.floor(baseWidth * 0.9);
		optimalHeight = Math.floor(baseHeight * 0.9);
	}

	// Calculate aspect ratio of the base dimensions
	const aspectRatio = baseWidth / baseHeight;

	// Calculate scale factors to fit the available space
	let widthScale = adjustedWidth / optimalWidth;
	let heightScale = availableHeight / optimalHeight;

	// Use the smaller scale to maintain aspect ratio
	let scale = Math.min(widthScale, heightScale);

	// Set minimum scale based on device type and provided option
	const defaultMinScale = isTouchDevice ? 0.5 : 0.6;
	scale = Math.max(scale, minScale || defaultMinScale);

	// For mobile devices, ensure we're not rendering unnecessarily large canvases
	if (isTouchDevice) {
		// Adjust max width based on performance tier - increased from original values
		const maxMobileWidth =
			performanceTier === 'high' ? 1024 : performanceTier === 'medium' ? 896 : 768;
	...
	// Return dimensions and scale factor
	return {
...
/**
 * Calculates the scaling factor for a canvas based on container dimensions and device
 * New helper function specifically for arcade cabinet UI
 */
export function calculateArcadeCabinetScale(
	gameWidth: number,
	gameHeight: number,
	...
	options?: {
		...
		sidePanelsTotalWidth?: number;
		padding?: number;
		minScale?: number;
	}
): { scale: number; effectiveWidth: number; effectiveHeight: number } {
	const { isTouchDevice, performanceTier } = deviceCapabilities;
	const {
		hasSidePanels = false,
		sidePanelsTotalWidth = 300,
		padding = isTouchDevice ? 16 : 8,
		minScale = isTouchDevice ? 0.5 : 0.6
	} = options || {};

	// Calculate available space
	let availableWidth = containerWidth;
	if (hasSidePanels) {
		availableWidth = availableWidth - sidePanelsTotalWidth;
	}
	...
	const widthScale = effectiveWidth / gameWidth;
	const heightScale = effectiveHeight / gameHeight;

	// Use the smaller scale to maintain aspect ratio
	let scale = Math.min(widthScale, heightScale);

	// Apply minimum scale based on device and performance
	const performanceMinScale =
		performanceTier === 'low'
			? minScale
			: performanceTier === 'medium'
				? minScale * 1.1
	...
	scale = Math.max(scale, performanceMinScale);
