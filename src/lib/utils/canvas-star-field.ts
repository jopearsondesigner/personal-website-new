...
export class CanvasStarFieldManager {
	...
	private initializeStars() {
		// Pre-allocate stars array with fixed size
		this.stars = new Array(this.starCount);

		// Create all stars at once to minimize allocations
	...
	setStarCount(count: number) {
		...
		if (count > this.stars.length) {
			const additionalCount = count - this.stars.length;
			for (let i = 0; i < additionalCount; i++) {
				this.stars.push(this.createStar());
			}
		}
		// If we need fewer stars, truncate the array
