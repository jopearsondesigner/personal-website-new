...
// Global Variables
// Ensure the canvas is correctly set up and added to the DOM
...
if (!canvas) {
  ...
  if (!ctx) {
    ...
    // Set a default background color to test if the canvas is working
    ctx.fillStyle = "white"; // Change 'white' to any visible color
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // Start drawing or animating after ensuring the canvas is visible
    console.log("Canvas is set up correctly. Starting draw loop...");
    // Your drawing or animation loop call here
  }
}
const ctx = canvas.getContext("2d");
canvas.width = 800;
...
class Spaceship {
  constructor() {
    // Initialize properties of the spaceship here
    this.x = canvas.width + 100; // Initial x position off-screen to the right
    this.y = Math.random() * (canvas.height / 2); // Random vertical position in the top half
    this.width = 100; // Adjust the width and height as needed
    this.height = 80;
    this.speed = Math.random() * 2 + 1; // Random speed between 1 and 3
    // Add any other properties or methods specific to your spaceship
  }

  update() {
    // Update the spaceship's position and other properties here
    this.x -= this.speed; // Move the spaceship from right to left
    // Add any other update logic for the spaceship
  }
  ...
  draw() {
    ...
    ctx.fillStyle = "rgba(255, 0, 0, 0.5)"; // Semi-transparent red color for a hazy effect
    ctx.fillRect(this.x, this.y, this.width, this.height);

    // Draw procedurally generated design
    this.design.forEach((part) => {
      ctx.fillStyle = part.color;

      if (part.type === 0) {
        // Rectangle
        ctx.fillRect(this.x + part.x, this.y + part.y, part.width, part.height);
      } else if (part.type === 1) {
        // Circle
        ctx.beginPath();
        ctx.arc(
          this.x + part.x + part.width / 2,
          this.y + part.y + part.height / 2,
          Math.min(part.width, part.height) / 2,
          0,
          2 * Math.PI
        );
        ctx.fill();
        ...
        // Triangle
        ctx.beginPath();
        ctx.moveTo(this.x + part.x, this.y + part.y + part.height);
        ctx.lineTo(this.x + part.x + part.width, this.y + part.y + part.height);
        ctx.lineTo(this.x + part.x + part.width / 2, this.y + part.y);
        ctx.closePath();
        ctx.fill();
      }
    });
    ...
    // Draw fire positions
    ctx.fillStyle = "rgba(255, 165, 0, 0.7)"; // Orange color for fire
    this.firePositions.forEach((pos) => {
      ctx.beginPath();
      ctx.arc(pos.x, pos.y, 5, 0, 2 * Math.PI); // Adjust size as needed
      ctx.fill();
    });
  }

  reset() {
    this.x = canvas.width + 100; // Reset position to off-screen right
    this.y = Math.random() * (canvas.height / 2); // Random vertical position in the top half
    this.speed = Math.random() * 2 + 1; // Random speed between 1 and 3
    this.fireRate = Math.random() * 500 + 500; // Random fire rate between 0.5 and 1 second
    this.fireTimer = this.fireRate; // Reset fire timer
    this.firePositions = []; // Clear fire positions
  }
}
...
window.addEventListener("keydown", function (e) {
  if (e.key === "Enter") {
    if (gameActive) {
      ...
      if (!titleScreen) {
        resetGame(); // Add the resetGame function implementation
      } else {
        titleScreen = false;
        gameActive = true;
        animate();
